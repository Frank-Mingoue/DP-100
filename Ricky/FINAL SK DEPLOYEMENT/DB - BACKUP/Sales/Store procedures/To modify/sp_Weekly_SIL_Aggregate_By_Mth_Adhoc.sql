IF  EXISTS (SELECT * FROM sys.procedures WHERE object_id = OBJECT_ID(N'[dbo].[sp_Weekly_SIL_Aggregate_By_Mth_Adhoc]'))
DROP PROCEDURE [dbo].[sp_Weekly_SIL_Aggregate_By_Mth_Adhoc]
GO 


--exec [sp_Weekly_SIL_Aggregate_By_Mth_Adhoc]
CREATE    PROCEDURE [dbo].[sp_Weekly_SIL_Aggregate_By_Mth_Adhoc] with	recompile
as
...
SET 				
		  @SP_NAME = 'sp_Weekly_SIL_Aggregate_By_Mth_Adhoc'; -- name of stored proc being executed		
...
-- End				
  ...
  IF EXISTS (SELECT  1 FROM WC_BU_D (NOLOCK) WHERE COUNTRY_CODE IN ( 'IT','ES','BE','NL','CH','WW','UA','FR', 'RO', 'CA','IE', 'PL','EC', 'CZ', 'SK','HU')) /* CR 6343*/
    BEGIN 
/*
	  --------------------------
