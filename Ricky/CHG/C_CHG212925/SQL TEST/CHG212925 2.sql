...
SELECT DISTINCT  SO.ORDER_NUM ,TP.NAME, OI.X_ORDER_UOM_TEXT ,OI.X_ORDER_QUANTITY_NUM ,S_ORDER_X.X_ORDER_CAT_CD
	,SO.X_SUB_TYPE
	,AC.NAME
	,AC.ALIAS_NAME
	,AC.X_ALIAS_NAME_2
...
--WHERE (SO.X_EXPORT_STATUS  <> 'Exported' OR SO.X_EXPORT_STATUS IS NULL) AND SO.STATUS_CD <> 'Pending' AND OI.QTY_REQ <> 0 AND BU.NAME = 'Belgium, BE'
WHERE (SO.X_EXPORT_STATUS IS NULL or SO.X_EXPORT_STATUS = 'In progress') 
AND SO.STATUS_CD IN ('Submitted', 'Returned') AND BU.NAME = 'Spain, ES'
 AND OI.QTY_REQ <> 0 
 AND OI.DLVRY_STATUS_CD <> 'Sent to Altadis' 
 AND TP.NAME IN ('Service Order', 'Stock Receive')
