...
FROM S_CONTACT
WHERE ROW_ID IN  ('7RZ-JS3','AO4-6INT','B8F-BE41','BF3-YB0Q','BF3-ZAZE','BGR-8ZHM','C0C-2ND5X','C0C-2TVL9','C0C-2TVLO','C0C-2TVM7','C0C-3GHVK','C0C-9NO2','C0C-AJP0','C0Y-1Q0HR','C5A-42Z0') --Contact UIDs provided by the client.
...
SELECT 'SELECT * INTO StagingDB.dbo.'+TAB+'_INC3913624_20211015  FROM ' + TAB+ ' WHERE '+COL +' IN(SELECT ROW_ID FROM StagingDB.dbo.CN_TO_BE_DELETED)'
FROM StagingDB.dbo.CN_IMPACTED_TAB_TO_BACKUP

--E.g
SELECT * INTO StagingDB.dbo.CX_TERR_INC3913624_20211015  FROM CX_TERR WHERE X_PR_EMP_ID IN(SELECT ROW_ID FROM StagingDB.dbo.CN_TO_BE_DELETED)
SELECT * INTO StagingDB.dbo.S_CONTACT_INC3913624_20211015  FROM S_CONTACT WHERE ROW_ID IN(SELECT ROW_ID FROM StagingDB.dbo.CN_TO_BE_DELETED)
SELECT * INTO StagingDB.dbo.S_CONTACT_BU_INC3913624_20211015  FROM S_CONTACT_BU WHERE CONTACT_ID IN(SELECT ROW_ID FROM StagingDB.dbo.CN_TO_BE_DELETED)
SELECT * INTO StagingDB.dbo.S_PARTY_INC3913624_20211015  FROM S_PARTY WHERE ROW_ID IN(SELECT ROW_ID FROM StagingDB.dbo.CN_TO_BE_DELETED)

 --Valid backup was correctly done. Replace with correct incident number and date.INCXXXXX_YYYYYMMDD 
 SELECT 'SELECT ''' + name+''' ,COUNT(1) FROM StagingDB.dbo.'+name FROM StagingDB.sys.objects WHERE name like '%_INC3913624_20211015%'

SELECT 'CX_TERR_INC3913624_20211015' ,COUNT(1) FROM StagingDB.dbo.CX_TERR_INC3913624_20211015
SELECT 'S_CONTACT_BU_INC3913624_20211015' ,COUNT(1) FROM StagingDB.dbo.S_CONTACT_BU_INC3913624_20211015
SELECT 'S_CONTACT_INC3913624_20211015' ,COUNT(1) FROM StagingDB.dbo.S_CONTACT_INC3913624_20211015
SELECT 'S_PARTY_INC3913624_20211015' ,COUNT(1) FROM StagingDB.dbo.S_PARTY_INC3913624_20211015


--Perform cleansing of EIM batch 
DELETE FROM EIM_CONTACT 
WHERE IF_ROW_BATCH_NUM BETWEEN 800000 AND 800999

--Populate EIM table with reference of records to be deleted 
INSERT INTO EIM_CONTACT (ROW_ID, IF_ROW_BATCH_NUM, IF_ROW_STAT, CON_PERSON_UID, CON_BU, PARTY_UID, PARTY_TYPE_CD)
SELECT S_CONTACT.ROW_ID, 800000+ ROW_NUMBER() OVER (ORDER BY S_CONTACT.ROW_ID)/1000, 'FOR_DELETE', S_CONTACT.PERSON_UID, S_BU.NAME, S_PARTY.PARTY_UID, S_PARTY.PARTY_TYPE_CD 
FROM S_CONTACT (NOLOCK)
INNER JOIN S_PARTY (NOLOCK)ON S_CONTACT.PAR_ROW_ID=S_PARTY.ROW_ID 
INNER JOIN S_BU (NOLOCK)  ON S_BU.ROW_ID=S_CONTACT.BU_ID 
WHERE S_CONTACT.ROW_ID IN (SELECT ROW_ID FROM StagingDB.dbo.CN_TO_BE_DELETED)

--Check EIM table
SELECT IF_ROW_BATCH_NUM, IF_ROW_STAT, COUNT(1)
FROM EIM_CONTACT(NOLOCK)
WHERE IF_ROW_BATCH_NUM BETWEEN 800000 AND 800999
